...
`github.com/Lorenzo-Holmes/cli_LH/v6/sdk/access` 包负责代理的入站访问认证。它提供一个轻量的管理器，用于按顺序链接多种凭证校验实现，让服务器在 CLI 运行时内外都能复用相同的访问控制逻辑。
...
通过 `go get github.com/Lorenzo-Holmes/cli_LH/v6/sdk/access` 添加依赖。
