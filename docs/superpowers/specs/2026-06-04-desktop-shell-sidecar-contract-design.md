...
`CLIProxyAPI` is a Go proxy core that provides OpenAI/Gemini/Claude/Codex-compatible APIs, provider adapters, OAuth flows, model registry management, token storage, load balancing, management APIs, request logging, and SDK entry points.

A desktop shell is an external application that presents the product UI and controls a local `CLIProxyAPI` process. `cockpit-tools` is a useful reference for this shape, but this repository should define a generic sidecar contract instead of importing a specific desktop product.
...
- Define a stable contract between external desktop shells and the `CLIProxyAPI` sidecar/core.
...
- Do not make `CLIProxyAPI` depend on `cockpit-tools` directories, account pages, tray logic, release workflow, or configuration schema.
...
The `CLIProxyAPI` core owns service behavior:
...
A shell may generate, edit, or select a `CLIProxyAPI` config file, but the core should not read shell-specific product configuration directly.
