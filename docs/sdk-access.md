...
The `github.com/Lorenzo-Holmes/cli_LH/v6/sdk/access` package centralizes inbound request authentication for the proxy. It offers a lightweight manager that chains credential providers, so servers can reuse the same access control logic inside or outside the CLI runtime.

## Importing

```go
import (
    sdkaccess "github.com/Lorenzo-Holmes/cli_LH/v6/sdk/access"
)
```

Add the module with `go get github.com/Lorenzo-Holmes/cli_LH/v6/sdk/access`.
...
```go
// configaccess is github.com/Lorenzo-Holmes/cli_LH/v6/internal/access/config_access
