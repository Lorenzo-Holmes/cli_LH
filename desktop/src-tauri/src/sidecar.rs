...
fn spawn_exit_watcher(
    ...
    std::thread::spawn(move || loop {
        ...
        let phase = if exit_status.success() {
            "stopped"
        } else {
            "error"
        }
...
pub fn rename_profile(
    ...
    if old_name.is_empty() || new_name.is_empty() {
        return Err("profile names are required".to_string());
    }
    ...
    let Some(profile) = profiles
        .iter_mut()
        .find(|item| item.name.eq_ignore_ascii_case(old_name))
    else {
        return Err(format!("profile not found: {old_name}"));
    };
    profile.name = new_name.to_string();
    write_profiles(&app, &mut profiles)
}

#[tauri::command]
pub fn delete_profile(app: AppHandle, name: String) -> Result<Vec<LaunchProfile>, String> {
    let name = name.trim();
    if name.is_empty() {
        return Err("profile name is required".to_string());
    }
    ...
    profiles.retain(|item| !item.name.eq_ignore_ascii_case(name));
...
pub fn discover_launch_profile(app: AppHandle) -> Result<DesktopSettings, String> {
    ...
    if settings.config_path.trim().is_empty() {
        if let Some(config) = find_first_existing(
            &roots,
...
pub fn open_app_data_dir(app: AppHandle) -> Result<(), String> {
    ...
    app.opener()
        ...
        .map_err(|err| format!("open app data dir: {err}"))
}
...
fn reveal_path(app: &AppHandle, path: &str, label: &str) -> Result<(), String> {
    ...
    app.opener()
        ...
        .map_err(|err| format!("reveal {label}: {err}"))
}
...
fn check_file_path(
    ...
    PreflightCheck {
        ...
        suggestion: Some(format!(
            "Choose an existing {label} path or run Auto-detect."
        )),
    }
}
...
fn parse_base_url_host_port(base_url: &str) -> Result<(String, u16), String> {
    ...
    let (host, port_text) = if authority.starts_with('[') {
        ...
        let port = rest
            .strip_prefix(':')
            .ok_or_else(|| "Base URL must include an explicit port".to_string())?;
        (host, port.to_string())
    } else {
    ...
    let port = port_text
        .parse::<u16>()
        .map_err(|_| "Base URL port must be between 1 and 65535".to_string())?;
    Ok((host, port))
}
...
fn replace_base_url_port(base_url: &str, port: u16) -> Result<String, String> {
    ...
    let (scheme, rest) = trimmed
        .split_once("://")
        .ok_or_else(|| "Base URL must include a scheme".to_string())?;
...
fn find_first_existing(roots: &[PathBuf], candidates: &[&str]) -> Option<PathBuf> {
    roots
        ...
        .flat_map(|root| {
            candidates
                .iter()
                .map(move |candidate| normalize_path(root.join(candidate)))
        })
